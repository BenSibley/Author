...
function ct_author_register_theme_page() {
	add_theme_page( sprintf( __( '%s Dashboard', 'author' ), wp_get_theme( get_template() ) ), sprintf( __( '%s Dashboard', 'author' ), wp_get_theme( get_template() ) ), 'edit_theme_options', 'author-options', 'ct_author_options_content', 'ct_author_options_content' );
}
...
function ct_author_options_content() {
	...
	<div id="author-dashboard-wrap" class="wrap">
		<h2><?php printf( __( '%s Dashboard', 'author' ), wp_get_theme( get_template() ) ); ?></h2>
		...
		<div class="content-boxes">
			...
			<?php if ( !function_exists( 'ct_author_pro_init' ) ) : ?>
				<div class="content content-premium-upgrade">
					<h3><?php printf( __( '%s Pro Plugin', 'author' ), wp_get_theme( get_template() ) ); ?></h3>
					<p><?php printf( __( 'Download the %s Pro plugin and unlock custom colors, featured videos, background images, and more', 'author' ), wp_get_theme( get_template() ) ); ?>...</p>
					<p>
						<a target="_blank" class="button-primary"
			...
			<div class="content content-review">
				...
				<p><?php printf( __( 'Help others find %s by leaving a review on wordpress.org.', 'author' ), wp_get_theme( get_template() ) ); ?></p>
